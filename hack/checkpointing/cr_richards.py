# modified version of richards which makes use of BLCR's checkpoint facilities
# to preserve the JITted code between multiple pypy invocations
#
# by antocuni
...
import os
def magic_run(fn, *args, **kwds):
    i = 0
    while True:
        fn(*args, **kwds) # run the main program
        res = os.system('cr_checkpoint --term %d' % os.getpid())
        if res != 0:
            return
        print 'restarting...', i
        i += 1
...
if __name__ == '__main__':
    import sys
    if len(sys.argv) >= 2:
        magic_run(main, iterations = int(sys.argv[1]))
    else:
        magic_run(main)
